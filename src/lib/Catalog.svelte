<script>
  ...
  function matchesQuery(item, query) {
    ...
    const matchesCatName = (item.categoria || item.categoria_nombre || '').toLowerCase().includes(query)
    const matchesDesc = (item.descripcion || '').toLowerCase().includes(query)
    const matchesBrand = Array.isArray(item.marcas)
      ? item.marcas.some(m => String(m).toLowerCase().includes(query))
      : (item.marcas || '').toLowerCase().includes(query)
    return matchesName || matchesCatName || matchesDesc || matchesBrand
  }
  ...
  $: filteredCatalog = catalog.filter((item) => {
    ...
    const itemCatNorm = normalizeCategory(item.categoria || item.categoria_nombre || '')

    const matchesCategory =
      itemCatNorm === selectedNorm ||
      itemCatNorm.includes(selectedNorm) ||
      selectedNorm.includes(itemCatNorm)
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
  <!-- Hero Section -->
  <div
    class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-950 via-slate-900 to-amber-950/60 border border-slate-800 p-8 md:p-12 shadow-2xl"
  >
    ...
    <div class="relative z-10 max-w-2xl space-y-4">
      <div
        class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-400/10 border border-amber-400/30 text-amber-300 text-xs font-extrabold uppercase tracking-wider shadow-sm"
      >
      ...
      <h1
        class="text-3xl md:text-5xl font-b text-white drop-shadow-md tracking-tight leading-tight"
      >
        Alquila la Maquinaria Pesada que tu Obra Necesita.
      </h1>

      <p
        class="text-sm md:text-base text-slate-200 leading-relaxed font-medium"
      >
        Excavadoras, Bulldozers, Retroexcavadoras y Grúas con disponibilidad
        inmediata. Cotiza tarifas por día, semana o mes con entrega en obra y
        asesoría inteligente por IA.
      </p>
      ...
      <div class="pt-3 flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
          <LucideIcons
            icon="search"
            class="absolute left-3.5 top-3.5 w-4 h-4 text-slate-400"
          />
          <input
            type="text"
  ...
  <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
    {#each categoriesList as cat}
      <button
        ...
        class="px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap border {selectedCategory ===
        cat
  ...
  {#if loading}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each Array(6) as _}
        <div
          class="h-96 rounded-2xl bg-surface border border-border animate-pulse p-4 flex flex-col justify-between"
        >
  ...
  {:else if filteredCatalog.length === 0}
    <div
      class="py-16 text-center space-y-3 bg-surface rounded-2xl border border-border"
    >
      ...
      <p class="text-xs text-text-faint max-w-sm mx-auto">
        Prueba buscando con otro término o seleccionando otra categoría en el
        menú superior.
      </p>
    </div>
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each filteredCatalog as item}
        <div
          class="group rounded-2xl bg-surface border border-border hover:border-amber-500/50 transition-all duration-300 overflow-hidden flex flex-col justify-between hover:shadow-xl hover:shadow-amber-500/5"
        >
          <!-- Image Header -->
          <div
            class="relative aspect-video overflow-hidden bg-bg cursor-pointer"
          ...
          >
            ...
            <span
              class="absolute top-3 left-3 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md bg-amber-500 text-slate-950 shadow"
            >
              {item.categoria}
            </span>
          ...
          <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
            <div>
              ...
              {#if item.specs}
                <div
                  class="mt-3 grid grid-cols-2 gap-1.5 text-[11px] text-text-muted bg-bg/50 p-2.5 rounded-xl border border-border/40"
                >
                  {#each Object.entries(item.specs).slice(0, 2) as [key, val]}
                    <div>
                      <span class="text-text-faint block text-[10px]"
                        >{key}</span
                      >
